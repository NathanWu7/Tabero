...
parser.add_argument(
    ...
    help=(
        "Output HDF5 path to save your manually recorded demos. "
        "This file can later be replayed/evaluated via replay_demos.py (as an input dataset)."
    ),
)
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Task suite to generate dataset for, select from: [xhumanoid, libero_10, libero_90, etc.].",
)
parser.add_argument(
    "--task_id",
    type=int,
...
parser.add_argument(
    "--create_multi_viewport", action="store_true", default=False, help="Create multi-viewport for recording."
)
parser.add_argument(
    "--recorder_type",
    type=str,
    ...
    choices=["default", "8d2"],
    help=(
        "Select recorder type for manual teleop recording:\n"
        "  default: IsaacLab default ActionStateRecorderManagerCfg\n"
        "  8d2    : Abs eef_pose (quat+pos, 7D) + binary gripper (1D) = 8D\n"
    ),
)
...
if "Libero" in args_cli.task or "Tactile" in args_cli.task:
...
from common.viewport_utils import create_new_viewports

import isaaclab_tasks  # noqa: F401
from isaaclab_tasks.utils.parse_cfg import parse_env_cfg

from tac_manip.core.recorders import AbsEEFPoseBinaryGripperActionStateRecorderManagerCfg
...
if args_cli.task_suite is not None:
    ...
    if args_cli.task_suite.startswith("libero"):
        # Keep --dataset_file as the primary/manual output path.
        # If user didn't override --dataset_file, allow env-driven outputs:
        # - RECORDED_DEMOS_PATH: write to a single explicit file
        # - RECORDED_DEMOS_DIR : write under a directory with task_suite/task_id naming
        if args_cli.dataset_file == "./datasets/dataset.hdf5":
            env_path = os.environ.get("RECORDED_DEMOS_PATH", "").strip()
            if env_path:
                args_cli.dataset_file = env_path
            else:
                env_dir = os.environ.get("RECORDED_DEMOS_DIR", "").strip()
                if env_dir:
                    out_dir_path = Path(env_dir).expanduser().resolve()
                    out_dir_path.mkdir(parents=True, exist_ok=True)
                    # Strict: directory mode must be resolvable by task_suite/task_id.
                    filename = f"{args_cli.task_suite}_task{args_cli.task_id}_recorded_demo.hdf5"
                    args_cli.dataset_file = str(out_dir_path / filename)
...
def run_simulation_loop(
    ...
    # create new assistant viewports for teleoperation
    if args_cli.teleop_device.lower() != "handtracking" and args_cli.create_multi_viewport:
        create_new_viewports(env.cfg)
